...
const loadAlgsFromInput = () => {
	...
	notifyUser(`${algDatabase.length} algs loaded, time to connect cube!`, 2000);
	return true;
};
...
const isSolved = (cube: Cube) => {
	...
	const isZBLSMode = (document.getElementById("isZBLSMode") as HTMLInputElement)
		.checked;
	...
	if (isZBLSMode) {
		...
		const bottomFaceSolidColour = cube[5].every(
			(sticker) => sticker === cube[5][8],
		);
